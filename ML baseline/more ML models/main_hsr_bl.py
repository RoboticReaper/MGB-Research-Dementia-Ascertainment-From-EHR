...
def load_text_data_csv(input_report, shuffle=False, filter_duplicate_instances=True):
    r''' load the csv train data: ../data/RAG_retrieved_notes_recursive_mpnet_recursive_mpnet.csv
        Args:
            input_report (string): input csv file directory
            shuffle (bool): whether to shuffle the dataset
            filter_duplicate_instances (bool): whether to remove duplicates
        Return:
            X, Y, id_list: lists containing clinical notes, binary labels, and empi IDs
    '''
    print(f"Start loading csv file from {input_report}.")
    ...
    for index, row in df.iterrows():
        the_id = str(row['empi'])
        the_x = str(row['notes'])
        the_y_raw = str(row['label']).strip().lower()
        ...
        if the_y_raw == 'yes':
            ...
            continue  # Skip rows with unrecognized labels
...
def nfold_hsr_arg(input_file, nfold=5, word_feature="FF", use_char=False, lr=0.2, cutoff=5):
    ...
    x, y, id_list = load_text_data_csv(input_file, shuffle=True)
    # x, y, id_list = load_text_data_csv(input_file, 'F', 'L', shuffle=True, filter_duplicacte_instances=False)
    ...
    for idx in range(nfold):
        ...
        config_file = "NCRFpp/adrd.config"
        ncrf = NCRF()
        ...
        ncrf.data.word_emb_dir = "../data/embeddings/gensim_word2vec_biowordvecpubmedmimic3d200.emb"
    ...
    fpr, tpr, roc_thresholds = metrics.roc_curve(test_all_gold, test_all_target_decode, pos_label=target_label)
    ...
    write_results("dl_output/aav_cohort/" + model_name + ".result.csv",
                  [pr_thresholds, precision, recall, roc_thresholds, fpr, tpr],
...
def run_hsr_arg(train_file, test_file, word_feature="FF", use_char=False, lr=0.2, cutoff=5):
    ...
    '''
    ## select the text and label from excel
    # train_x, train_y, train_ids = load_text_data_csv(train_file, 'G', 'M', shuffle=True)
    # test_x, test_y, test_ids = load_text_data_csv(test_file, 'G', 'M', filter_duplicacte_instances=False)
    train_x, train_y, train_ids = load_text_data_csv(train_file, shuffle=True)
    test_x, test_y, test_ids = load_text_data_csv(test_file)
    ...
    target_single_prob = np.asarray(target_prob)

    # Safely extract the probabilities for the 'yes' (1) class
    if target_single_prob.ndim == 2:
        test_all_target_decode = target_single_prob[:, target_id]
    else:
        # Fallback if NCRF++ already flattened the predictions
        test_all_target_decode = target_single_prob
    ...
    plot_precision_recall(precision, recall, model_name,
                          "dl_output/generalizability_output/updated_v2/" + model_name + ".prc.jpg")
    plot_precision_recall(precision, recall, model_name,
                          "dl_output/generalizability_output/updated_v2/" + model_name + ".prc.jpg")
    plot_roc(fpr, tpr, auc, model_name, "dl_output/generalizability_output/updated_v2/" + model_name + ".roc.jpg")
    write_results("dl_output/generalizability_output/updated_v2/" + model_name + ".result.csv",
                  [pr_thresholds, precision, recall, roc_thresholds, fpr, tpr],
                  ["P/R Thresholds", "Precision", "Recall", "ROC Thresholds", "FPR", "TPR"], auc)
    print("auroc:", auc)
    print("auprc:", auprc)
...
def compute_metrics(prob_file):
    ...
    write_results("dl_output/generalizability_output/" + model_name + ".result.csv",
                  [pr_thresholds, precision, recall, roc_thresholds, fpr, tpr],
                  ["P/R Thresholds", "Precision", "Recall", "ROC Thresholds", "FPR", "TPR"], auc)
...
if __name__ == '__main__':
    # python main_hsr_bl.py LSTM T 0.2 5
    # ncrf = NCRF()
    ...
    input_file = r"../data/RAG_retrieved_notes_recursive_mpnet.csv"
    ...
    # exit(0)
    train_file = r"../data/RAG_retrieved_notes_recursive_mpnet.csv"
    test_file = r"../data/RAG_retrieved_notes_recursive_mpnet.csv"
