...
# @Last Modified time: 2019-04-18 16:48:29
# -*- coding: utf-8 -*-
...
def show_distribution(input_list):
    ...
    for a in input_list:
        if a in the_dict:
            ...
            the_dict[a] = 1 
    for key, value in sorted(the_dict.items(), key=lambda kv: (kv[1], kv[0])):
        print("\t\t%s: %s" % (key, value))


def filter_duplicate(input_list, indent='\t', filter_id=0):
...
def plot_multiple_classification_results_pp(input_list, model_titles, save_dir=None):
    ...
    # plt.title('PositiveNum-ProbabilityThreshold Curve (165,962 reports in total)')

    for idx in range(result_num):
        print(input_list[idx])
        # plt.plot(input_list[idx][0], input_list[idx][1], color_list[idx],
        #          label=f'{input_list[idx][2]} ({input_list[idx][-1]})')
        # plt.plot(input_list[idx][0], input_list[idx][1], color_list[idx], label='%s' % (input_list[idx][2]))
        plt.plot(input_list[idx][0], input_list[idx][1], color_list[idx], label='%s' % (model_titles[idx]))
    # plt.plot([0, 1], [0, 1],'r--')
    # plt.legend(loc='best', title='AUC Scores')
    plt.legend(loc='best')
    plt.xlim([0, 1.01])
    # plt.ylim([0, 1.01])
    ...
    ytick = [(x+0.)/10 for x in range(0, 11)]
...
if __name__ == '__main__':
    ...
    load_all_decode_result("all_extendtraining.csv")
