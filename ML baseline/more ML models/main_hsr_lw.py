...
def nfold_hsr_arg(input_file, nfold=5, word_feature="FF", use_char=False, lr=0.2, cutoff=5):
    ...
    x, y, id_list = load_text_data_xlsx(input_file, 'G', 'M', shuffle=True)
    ...
    for idx in range(nfold):
        ...
        ## Change with the suggestion from Jie
        target_id = ncrf.data.label_alphabet.get_index('1') ## added
        target_single_prob = np.asarray(target_prob) ## added
        
        
        #test_decode_list.append(np.asarray(target_prob))
        test_decode_list.append(target_single_prob[:,target_id]) # updated
    ...
    test_all_decode = np.concatenate(test_decode_list) # updated according to Jie's suggestion
    ...
    test_all_target_decode = test_all_decode
...
if __name__ == '__main__':
    ...
    with open('dl_output/dataset_i_output/pred_probs_final_20220329.csv', 'w', newline='', encoding='utf-8') as f:
