...
def train_decode(input_file):
    ...
    train_x, dev_x, test_x = x[:15], x[15:17], x[17:]
    train_y, dev_y, test_y = y[:15], y[15:17], y[17:]
    ...
    # config_file = "NCRFpp/demo.train.config"
    ...
    ncrf.data.word_emb_dir = "NCRFpp/sample_data/sample.word.emb"
    ...
    ncrf.train(None, "dl_test_output/att.model")
...
def adr_hsr_decode(model_file, decode_file, output_file):
    ...
    test_x, test_y = load_text_classification_data_xlsx(decode_file,"E",'D', True)
    test_x = test_x[:1000]
    testy = test_y[:1000]
