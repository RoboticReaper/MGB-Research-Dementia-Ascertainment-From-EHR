...
def evaluate(data, model, name, nbest=0):
    ...
    for batch_id in range(total_batch):
        ...
        if nbest>0 and not data.sentence_classification:
    ...
    try:
        ...
        speed = 'decode time too small, division by zero'
    ...
    if name == "test":
        fout = open("in.txt",'w')
        for inp, lab in zip(instances, pred_results):
            # print(inp)
            fout.write(lab +" | "+" ".join([str(a) for a in inp[0]])+"\n")
