...
# @Last Modified time: 2019-06-06 13:56:47
# -*- coding: utf-8 -*-
from openpyxl import load_workbook
import matplotlib.pyplot as plt
import numpy as np
import csv
from utils import clinic_text_processing, filter_duplicate
import random
import copy
...
def load_text_classification_data_csv(input_report, column_list, shuffle=False, filter_duplicacte_instances=True, include_title=True):
    ...
    if filter_duplicacte_instances:
        output_list = filter_duplicate(output_list, '\t', -1)
    return output_list
...
def load_multi_text_classification_data_xlsx(input_report, X_id, Y_id, new_Y_id, shuffle=False, filter_duplicacte_instances=True, include_title=True):
    ...
    if filter_duplicacte_instances:
        X,Y, new_Y = filter_duplicate([X,Y, new_Y])
    instance_num = len(X)
    print("\tInstance Num: %s"%instance_num)
    if shuffle:
...
def load_pair_txt_file(input_file):
    ...
    if filter_duplicacte_instances:
        record_id_list,record_list = filter_duplicate([record_id_list,record_list])
...
if __name__ == '__main__':
    count_avg_sentence_length("decode/HSR.BWH.att.trainall.decode.xlsx", "D")
    exit(0)

    BWH_all_data = r'../Data/BWH_keyvalue_desc_124230_Cases_04012019.txt'
    load_pair_txt_file(BWH_all_data)
    exit(0)

    snm= "Decode_MergeTraining636_NoteTextData_Grp_all.csv"
    out_file ="NoteText_Data_Grp_all.validate.csv"
    train_file = "Data_merged.csv"
    out_file = "NoteText_Data_Grp_all.filtertraining.csv"
    select_filter_train_to_annotate(snm, train_file, out_file)
    # select_low_to_annotate(snm, out_file)
    exit(0)
    print("Formatfactory")

    # extend_lexicon_data = r"../keyvalue_desc_174799cases_0118019_mtermlexicon_remove_annotated_cases_4.csv"
    # load_text_classification_data_csv(extend_lexicon_data,[8])
    decoded_file = "mtermslexicon.csv"
    select_medium_to_annotate(decoded_file,'mterms+.medium.annotate.csv')
    exit(0)
    all_decode_file = '../final-annotated.models.decode.csv'
    select_top_to_annotate(all_decode_file,'all.top100.annotate.csv')
